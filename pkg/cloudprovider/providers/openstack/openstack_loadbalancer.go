...
func (os *OpenStack) GetTCPLoadBalancer(name, region string) (*api.LoadBalancerStatus, bool, error) {
	vip, err := getVipByName(os.network, name)
	if err == ErrNotFound {
...
func (os *OpenStack) EnsureTCPLoadBalancer(name, region string, loadBalancerIP net.IP, ports []*api.ServicePort, hosts []string, affinity api.ServiceAffinity) (*api.LoadBalancerStatus, error) {
	...
	_, exists, err := os.GetTCPLoadBalancer(name, region)
	if err != nil {
	...
	if exists {
		...
		err := os.EnsureTCPLoadBalancerDeleted(name, region)
		if err != nil {
	...
	glog.V(4).Infof("CreateTCPLoadBalancer monitor")
...
func (os *OpenStack) UpdateTCPLoadBalancer(name, region string, hosts []string) error {
	...
	err = pager.EachPage(func(page pagination.Page) (bool, error) {
		...
		for _, member := range memList {
			if _, found := addrs[member.Address]; found {
				...
				err = secgroups.RemoveServerFromGroup(os.compute, server.ID, name).ExtractErr()
				if err != nil {
					return false, err
				}
			}
	...
	for addr := range addrs {
		...
		err = secgroups.AddServerToGroup(os.compute, server.ID, name).ExtractErr()
		if err != nil {
			return err
		}
	}
...
func (os *OpenStack) EnsureTCPLoadBalancerDeleted(name, region string) error {
	...
	if vip != nil {
		...
		pool, err = getPoolByName(os.network, name)
		if err != nil && err != ErrNotFound {
			return err
		}
	}
	...
	secgroup, err := getSecGroupByName(os.compute, name)
	if err != nil && err != ErrNotFound {
		return err
	}
	if secgroup != nil {
